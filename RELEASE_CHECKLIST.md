...
- [x] Run `geogrid.exe`, combined `ungrib.exe` with `Vtable.ECMWF`, and
  `metgrid.exe`.
- [x] Confirm all expected `met_em.d01` hours are present with the combined
  preprocessing route.
- [x] Run `real.exe` and verify `wrfinput_d01` and `wrfbdy_d01` after that
